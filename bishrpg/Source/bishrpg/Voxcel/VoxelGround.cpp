...
// KeepSimulationChanges begin
//#include "Modules/ModuleManager.h"
//#include "LevelEditor.h"
//#include "Engine/Selection.h"
// KeepSimulationChanges end
...
AVoxelGround::AVoxelGround()
{
	...
	if(SceneComponent) {
		//SceneComponent->UpdatedComponent = CapsuleComponent;
	}
...
// コンストラクションスクリプト
void AVoxelGround::OnConstruction(const FTransform& Transform)
{
	Super::OnConstruction(Transform);

	GAME_LOG("AVoxelGround::OnConstruction");
}
...
// Called every frame
void AVoxelGround::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);
	
	if(GetWorld()->GetFirstPlayerController()->InputKey(EKeys::O, EInputEvent::IE_Pressed, 0.5f, false)) {
		GAME_LOG("AVoxelGround::Pressed X");
		SetActorLocation(FVector(100, 100, 100), false, nullptr, ETeleportType::None);
		SaveSimulationChanges();
	}
}

void AVoxelGround::SaveSimulationChanges()
{
	AActor* EditorWorldActor = EditorUtilities::GetEditorWorldCounterpartActor(this);
	if(EditorWorldActor != NULL) {
		...
		const int32 CopiedPropertyCount = EditorUtilities::CopyActorProperties(this, EditorWorldActor, CopyOptions);
	}
	
}

#if 0
void AVoxelGround::OnKeepSimulationChanges()
{
...
bool AVoxelGround::SaveAnimationFromSkeletalMeshComponent(AActor * EditorActor, AActor * SimActor, TArray<class USkeletalMeshComponent*> & OutEditorComponents)
{
